    ...
    <div className="flex-1 overflow-y-auto">
      {conversations.map((conv) => (
        <div
          ...
          className={`flex items-center p-4 cursor-pointer transition-all duration-200 border-b border-gray-100 last:border-b-0 ${
            selectedId === conv.id ? 'bg-indigo-50/80 border-l-4 border-l-indigo-600' : 'hover:bg-gray-100/80 border-l-4 border-l-transparent'
          }`}
        >
          <div className="relative mr-4">
            ...
            {conv.type === 'direct' && (
              <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${
                conv.participants.find(p => p.id !== currentUserId)?.status === 'online' ? 'bg-green-500' : 'bg-gray-400'
              }`}></div>
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex justify-between items-center">
              ...
              {conv.unread_count > 0 && (
                <span className="ml-2 bg-indigo-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full min-w-[20px] text-center">
                  {conv.unread_count}
                </span>
              )}
            </div>
